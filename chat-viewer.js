...
function renderSharedChatViewer(messages) {
    const signature = JSON.stringify(
        messages.map(item => [item.message_id, item.sent_at])
    );

    if (signature === sharedChatViewerSignature) {
        return;
    }

    sharedChatViewerSignature = signature;
    ...
    if (messages.length === 0) {
        ...
        sharedChatList.appendChild(empty);
        return;
    }
    ...
    messages.forEach(item => {
        ...
        time.textContent = formatSharedChatTime(item.sent_at);

        const text = document.createElement("div");
        text.className = "shared-chat-text";
        text.textContent = item.message_text || "";

        meta.appendChild(name);
        meta.appendChild(time);
        card.appendChild(meta);
        card.appendChild(text);
        sharedChatList.appendChild(card);
    });

    if (nearBottom || sharedChatList.scrollTop === 0) {
        sharedChatList.scrollTop = sharedChatList.scrollHeight;
    }
}
...
async function loadSharedChatViewer() {
    ...
    const sessionId = getSharedChatSessionId();
    ...
    try {
        ...
        renderSharedChatViewer(messages);

    } catch (error) {
        console.error("共有チャット取得エラー:", error);

        sharedChatStatus.textContent =
            "チャットを取得できませんでした";

    } finally {
        sharedChatViewerLoading = false;
    }
}


function setModeratorChatFeedback(text, isError = false) {
    if (!moderatorChatMessage) {
        return;
    }

    moderatorChatMessage.textContent = text;
...
async function sendModeratorChatMessage() {
    ...
    moderatorChatSending = true;
    moderatorChatInput.disabled = true;
    moderatorChatSendButton.disabled = true;
    moderatorChatSendButton.textContent = "送信中…";
    setModeratorChatFeedback("");
    ...
    try {
        ...
        moderatorChatSending = false;
        moderatorChatInput.disabled = false;
        moderatorChatSendButton.disabled = false;
        moderatorChatSendButton.textContent = "送信";
